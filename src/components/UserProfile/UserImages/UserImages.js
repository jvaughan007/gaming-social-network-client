// this is an extended feature and is not commented out for testing purposes
...
const UserImages = ({ profile, userIsOwner }) => {
  ...
  useEffect(() => {
    const getAllImages = async () => {
      try {
        const res = await fetch(`${API_URL}/profiles/${user.username}/images`);
      ...
      return (
        <div className='image-div'>
          {images.map((image, y) => {
            return (
              <div key={y}>
                <img src={image.image_url} alt='User Img'></img>
              </div>
            );
          })}
        </div>
